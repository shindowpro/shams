(function ($) {
    "use strict"; $.fn.mySelect = function (options) {
        let $this = $(this),
        ...
        styledSelect.attr("data-value",
            $this.children("option").eq(0).val());
    ...
    function menuVertical() {
        ...
        if (window_width <= breakPoint) {
            if (dropDonw_tigger.hasClass(activeClass)) { dropDonw_tigger.removeClass(activeClass) }
            if (dropdown.hasClass(activeClass)) { dropdown.removeClass(activeClass) }
        } else { dropDonw_tigger.addClass(activeClass); dropdown.addClass(activeClass) }
    }
    function innerPageBreadcumb() {
        let promotion = $(".xs-promotion"),
            transparentHeader = $(".header-transparent"),
            totalHeight = Math.floor(promotion.outerHeight(!0) + transparentHeader.outerHeight(!0));
    ...
    function mobileMenu() {
        ...
        if ($(window).width() <= 991) {
            ...
            if ($(header).hasClass("mobile-menu")) { $(header).removeClass("mobile-menu") }
        }
    }
    ...
    $(window).on("load", function () {
        ...
        if ($(".dokan-widget-area").length > 0) {
            console.log("test"); $(".dokan-widget-area").addClass("sidebar")
        }
        ...
        if ($(".marketo-xs-header-5").length > 0) {
            ...
            } else { $("body").removeClass("xs-header-5") }
        }
    });
    $(document).ready(function () {
        ...
        $(".xs-countdown-timer[data-countdown]").each(function () {
            ...
            $this.countdown(finalDate, function (event) {
                var $this = $(this).html(event.strftime(" " + '<div class="xs-timer-container"><span class="timer-count">%-D </span><span class="timer-title">' + xs_product_timers.xs_date + "</span></div>" + '<div class="xs-timer-container"><span class="timer-count">%H </span><span class="timer-title">' + xs_product_timers.xs_hours + "</span></div>" + '<div class="xs-timer-container"><span class="timer-count">%M </span><span class="timer-title">' + xs_product_timers.xs_minutes + "</span></div>" + '<div class="xs-timer-container"><span class="timer-count">%S </span><span class="timer-title">' + xs_product_timers.xs_secods + "</span></div>"))
            })
        });
        ...
        if ($(".nav-hidden-menu").length > 0) {
            $(".nav-hidden-menu").xs_nav({ hidden: !0, });
        ...
        $(".rate-graph").each(function () {
            if ($(this).find(".rate-graph-bar").attr("data-percent") <= 100) {
                $(this).find(".rate-graph-bar").css({ width: $(this).find(".rate-graph-bar").attr("data-percent") + "%", })
            } else { $(this).find(".rate-graph-bar").css({ width: 100 + "%", }) }
        });
        ...
        echo.init({ offset: 100, throttle: 100, unload: !1, });
        if ($(".xs-nav-tab li a").length > 0) {
            $(".xs-nav-tab li a").on("click", function () { echo.render() })
        }
        if ($(".xs-newsletter").length > 0) {
            var mailchimp_url = $(".xs-newsletter").data("link");
            $(".xs-newsletter").ajaxChimp({ url: mailchimp_url, })
        }
        ...
        if ($(".waypoint-tigger").length > 0) {
            var waypoint = new Waypoint({
                ...
                handler: function (direction) { animateProgressBar() },
                offset: "50%",
            })
        }
        ...
        $(".offset-cart-menu").on("click", function (e) {
            e.preventDefault(); $(".xs-sidebar-group").addClass("isActive")
        });
        ...
        $(document).on("click", ".ajax-search-result, .xs-navbar-search", function (e) { e.stopPropagation() });
        $(".xs-dropdown-trigger").on("click", function (e) { e.stopPropagation() });
        ...
        if ($(window).width() >= 1024) {
            if ($(".xs-dropdown-trigger").length > 0) {
                $(".xs-dropdown-trigger").on("click", function () {
                    if ($(".nav-cover").hasClass("dropdown-is-active")) {
                        $(".nav-cover").removeClass("dropdown-is-active")
                    } else { $(".nav-cover").addClass("dropdown-is-active") }
                })
            }
        }
        if ($(".elementskit-navbar-nav-default .ekit-menu-badge-arrow").length > 0) {
            $(".elementskit-navbar-nav-default .ekit-menu-badge-arrow").each(function () {
                ...
                $(this).css({ "border-left-color": left_color, })
            })
        }
    });
    $(window).on("scroll", function () { });
    $(window).on("resize", function () {
        innerPageBreadcumb(); menuVertical(); mobileMenu(); headerRemove()
    });
    ...
    userInteractionEvents_Preload.forEach(function (e) {
        window.addEventListener(e, triggerScriptLoader_Preload, { passive: !0, })
    });
    if (fontList) {
        ...
        Promise.all([rubikObserver.load(), robotoObserver.load()]).then(function () { document.documentElement.className += " fonts-loaded" })
    }
})(jQuery)