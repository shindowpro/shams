(function ($) {
    "use strict"; $('body').on('submit', 'form.cart', function (e) {
        ...
        $.post(xs_ajax_obj.ajaxurl, data, function (response) {
            ...
            if (typeof (wc_add_to_cart_params) !== 'undefined' && wc_add_to_cart_params !== null && wc_add_to_cart_params.cart_redirect_after_add === 'yes') {
    ...
    $(document).on('added_to_wishlist removed_from_wishlist', function () {
        $.ajax({
            ...
            data: { action: 'marketo_wishlist_count' },
            method: 'POST',
    ...
    search_form.on('keyup', 'input[type="search"]', function (event) {
        switch (event.which) {
            case 27: $(this).parents('.xs-navbar-search-wrapper').find('.ajax-search-result').remove(); $(this).val('');
                break;
            default: if (timer) clearTimeout(timer);
                timer = setTimeout(() => { xp_search($(this).parents('.xs-navbar-search-wrapper')) }, 800)
                break
        }
    });
    $('.xs-navbar-search-wrapper').submit(function (e) {
        e.preventDefault(); xp_search($(this))
    });
    function xp_search($this) {
        ...
        if (keyword.length > 0 || cat_id) {
            ...
            $.ajax({
                url: xs_ajax_obj.ajaxurl,
                data: 'action=xp_result_search_product&keyword=' + keyword + '&cate_id=' + cat_id, method: 'POST',
                success: function (data) {
                    $this.find('.xs-spin').removeClass('xsicon xsicon-spinner-solid fa-spin');
                    var html = '';
                    var sHtmlViewMore = ''; if (data) {
    ...
    var helper = {
        debounce: function (fn, delay) {
            ...
            return function (...args) {
                timeID ? clearTimeout(timeID) : null; timeID = setTimeout(function () { fn(...args) }, delay)
            }
        },
    ...
    if (ajax_load_more_product.enabled == '1') {
        ...
        var container = {
            ...
            pagi: $('.woocommerce-pagination'), style: 'load-more-on-scroll',
        };
        if (container.style === 'load-more-on-scroll') {
            $(document).on('scroll', helper.debounce(function () {
                if (helper.isInViewport(container.pagi[0])) {
                    ...
                    if (link) { loadProduct(container.style, link, !0) }
                }
            }, 500))
        }
        function loadProduct(style, link, onscroll) {
            $.ajax({
                url: link, beforeSend: function () {
                    container.main.addClass('is-loading'); container.pagi.after('<p class="ajax-loading"><i class="xsicon xsicon-spinner"></i></p>')
                },
                success: function (response) {
                    if (style === 'load-more-on-scroll' && onscroll) {
                        let product = $(response).find('.feature-product-v4 .row').first();
                        let pagi = $(response).find('.woocommerce-pagination'); container.product.append(product.html());
                        container.pagi.html(pagi.html())
                    }
                },
                complete: function () { container.main.removeClass('is-loading'); $('.ajax-loading').remove() },
            })
        }
    }
}(jQuery))